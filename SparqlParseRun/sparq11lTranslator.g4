...
int i = q.ResultSet.Variables.Values.ToList().IndexOf($var.p);
	Func<SparqlResult, dynamic> orderFunc = pac=>
						{	
                            if (!(pac[i] is ILiteralNode))
                                return pac[i];
                            else
                            {
                                var l = (ILiteralNode)pac[i];
                                //if (l.LiterType is double)
                                //    return double.Parse(l.Value);
                                //if (l.Value is long)
                                //    return long.Parse(l.Value);
                                return l.ToString();
...
( ( graphPatternNotTriples { $value.Triples.AddRange($graphPatternNotTriples.value.Triples);  }
...
graphPatternNotTriples	returns [SparqlWhere value] 
...
:'OPTIONAL' {int parametersStartIndex = q.ResultSet.Variables.Count; } groupGraphPattern
{
	...
	$value.Triples= $groupGraphPattern.value.Triples;
	 ...
	 $value.EndIndex = q.ResultSet.Variables.Count-parametersStartIndex;
} ;


graphGraphPattern	returns [SparqlWhere value] :'GRAPH' varOrIRIref groupGraphPattern;


groupOrUnionGraphPattern	returns [UnionWhere value] : 
{short lastKnownIndex = (short)q.ResultSet.Variables.Count;}
first=groupGraphPattern {$value=new UnionWhere($first.value);}
 (
 {  foreach(var newV in q.ResultSet.Variables.Skip(lastKnownIndex))
	newV.Value.isNew=true; }
 'UNION' second=groupGraphPattern { $value.Add($second.value);})*   
 { foreach(var newV in q.ResultSet.Variables.Skip(lastKnownIndex))
...
constructTemplate 
:'{' (constructTriples { q.CreateConstructTemplate($constructTriples.value); } )? '}'  ;

constructTriples  returns [SparqlWhere value=new SparqlWhere()]	 
:	triplesSameSubject	 {$value.Triples.AddRange($triplesSameSubject.value.Triples);} 
( '.' (next= constructTriples {$value.Triples.AddRange($next.value.Triples); })? )?	;
...
:	IRI_REF  	{	
		$value  = new SparqlUriNode{ Uri = new Uri($IRI_REF.text) };	
		//iri = iri.Substring(1, iri.Length - 2);
