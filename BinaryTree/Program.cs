...
namespace BinaryTree
{
    public class Programm
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("======Count() ok. duration=" + (DateTime.Now - tt0).Ticks/10000L);
            tt0 = DateTime.Now;
        ...
        private static BTree TestQueryInput(IEnumerable<object[]> query, PType ptElement,
            Func<object, PxEntry, int> edepth, string path)
        {
        ...
        private static void TestTreeOfInt(IEnumerable<object[]> query, string path)
        {
            ...
            Console.WriteLine(finded.Field(0).Get() + " finded, duration=" + (DateTime.Now - tt0).TotalMilliseconds +
                              "ms");
            Console.WriteLine();
            tree.Close();
        }

        private static void SimpleTreeInt(string path)
        {
        ...
        private static void AddTreeAddChart(string path, Stopwatch timer, object[] objects, int[][] results, int j)
        {
            ...
            foreach (var pair in objects)
                cell.Add(pair);
            timer.Stop();
            Console.WriteLine("simple int tree " + objects.Length + "elements created by add per one for (ms)" +
        ...
        private static void TestGetByKey(BTreeInt tree, int max, PaCell paCell)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                timer.Stop();
                Console.WriteLine("search in " + max + " elements TREE find " + tested + " for (ticks)" +
                                  timer.Elapsed.Ticks);
                timer.Restart();
                var res1 = paCell.Root.BinarySearchFirst(entry => (int) entry.Get() - tested);
                timer.Stop();
                Console.WriteLine("binary search in " + max + " elements PA CELL find " + tested + " for (ticks)" +
                                  timer.Elapsed.Ticks);
                Console.WriteLine();
            }
        }
