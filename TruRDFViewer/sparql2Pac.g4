...
orderClause	returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value]
 :'ORDER' 'BY' main= orderCondition { $value = $main.value; }
  (others = orderCondition { var valueClone=$value.Clone()  as Func<IEnumerable<RPack>,IEnumerable<RPack>>; var othersRes=$others.value; $value = packs => othersRes(valueClone(packs)); })*	;


orderCondition	 returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value, bool isDescending]
:  { q.currentFilterParameter = Expression.Parameter(typeof (RPack)); }
...
groupGraphPattern	 returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value]	
:'{' (strt=triplesBlock {$value=$strt.value;})? 

( ( graphPatternNotTriples  
		{ if($value==null) { $value=$graphPatternNotTriples.value;  }
					else { var valueClone=$value.Clone() as Func<IEnumerable<RPack>,IEnumerable<RPack>>;
	...
	(end=triplesBlock 
		{	var valueClone=$value.Clone() as Func<IEnumerable<RPack>,IEnumerable<RPack>>;
			var endvalue=$end.value; $value=packs=>endvalue(valueClone(packs)); }  
	)? 
)* '}'	;


triplesBlock returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value]	 
...
graphPatternNotTriples	returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value] 
...
optionalGraphPattern returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value]	 
...
graphGraphPattern	returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value] :'GRAPH' varOrIRIref groupGraphPattern;


groupOrUnionGraphPattern	returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value] : 
...
filter		returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value]  	
:'FILTER' { q.currentFilterParameter = Expression.Parameter(typeof (RPack));}  
constraint
{
var f=Expression.Lambda<Func<RPack, bool>>($constraint.value, q.currentFilterParameter).Compile();
...
triplesSameSubjectConstruct	 returns [Func<RPack,IEnumerable<Tuple<string,string,string>>> value]
locals [Func<RPack, string> subj=null]
...
propertyListNotEmptyConstruct returns [Func<RPack, IEnumerable<Tuple<string,string,string>>> value]
: main= verbObjectListConstruct { $value=$main.value; } 
( ';'(seconds = verbObjectListConstruct {  var valueClone=$value.Clone() as Func<RPack,IEnumerable<Tuple<string,string,string>>>;var scnds=$seconds.value; $value=packs=>scnds(packs).Concat(valueClone(packs)); } )?)*  ;

verbObjectListConstruct  returns [Func<RPack, IEnumerable<Tuple<string,string,string>>> value]
locals[Func<RPack, string> pred=null]
...
objectListConstruct returns [Func<RPack, IEnumerable<Tuple<string,string,string>>> value]
 : o0=graphNodeConstruct { var o0value= $o0.value; $value=pack => Enumerable.Repeat(o0value(pack),1);	 } 
( ',' o1=graphNodeConstruct {  var valueClone=$value.Clone() as Func<RPack,IEnumerable<Tuple<string,string,string>>>; var o11=$o1.value; $value=packs=>valueClone(packs).Concat(Enumerable.Repeat(o11(packs),1)); } )*;

graphNodeConstruct returns [Func<RPack, Tuple<string,string,string>> value]
: varOrTermConstruct	{ $value=$varOrTermConstruct.value; }
|	triplesNode	;

 varOrTermConstruct returns [Func<RPack, Tuple<string,string,string>> value]
: varLiteral 
{	   
...
triplesSameSubject	 returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> value]
locals [Variable subj=new Variable()]
:	varOrTermSub propertyListNotEmpty  
{  $value=$propertyListNotEmpty.f; } 
|	triplesNode propertyList	{} ;

propertyListNotEmpty  returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> f]
: main= verbObjectList  { $f=$main.f; } 
( ';'(seconds = verbObjectList { var valueClone=$f.Clone() as Func<IEnumerable<RPack>,IEnumerable<RPack>>; var scnds=$seconds.f; $f=packs=>scnds(valueClone(packs)); } )?)*  ;

propertyList returns	[List<VarOrTermContext> value]	  : (propertyListNotEmpty {  })?;

verbObjectList  returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> f]
locals[Variable PredicateVariable]
: verb objectList { $f=$objectList.f; } ;

objectList returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> f]
 : o0=graphNode { $f=$o0.f;	 } 
( ',' o1=graphNode { var valueClone=$f.Clone() as Func<IEnumerable<RPack>,IEnumerable<RPack>>; var o11=$o1.f; $f=packs=>o11(valueClone(packs)); } )*;

verb
: varOrIRIref { $verbObjectList::PredicateVariable=$varOrIRIref.p; } 
| 'a' {	
var PredicateVariable=new Variable();							   
PredicateVariable.pacElement =  "http://www.w3.org/1999/02/22-rdf-syntax-ns#type";  
PredicateVariable.graph	= new GraphIsDataProperty(){IsData=false};
$verbObjectList::PredicateVariable=PredicateVariable;	   
} ;
											
triplesNode	 returns	[Func<IEnumerable<RPack>, IEnumerable<RPack>> f] 
 :	collection { }
 |	blankNodePropertyList {};

blankNodePropertyList	returns	[Func<IEnumerable<RPack>, IEnumerable<RPack>> f] :'[' propertyListNotEmpty ']' {	} ;

collection	 returns	[Func<IEnumerable<RPack>, IEnumerable<RPack>> f] :'(' ( graphNode {	 } )+ ')';

graphNode  returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> f]
...
| graphTerm  {	$triplesSameSubject::subj.pacElement =  $graphTerm.entityCode; };
 ...
 varOrTerm returns [Func<IEnumerable<RPack>, IEnumerable<RPack>> f]
: var 
{	   
...
graphTerm returns[string entityCode, Literal d]	 
:	iRIref 	{   $entityCode = $iRIref.value; } 
...
|	BlankNode  { $entityCode = $BlankNode.text; } 
...
: iRIref { var code=$iRIref.value; $value = Expression.Constant(code);}
