...
namespace TruRDFViewer
{
    ...
    public class Query
    {                   
        internal Func<IEnumerable<RPack>, IEnumerable<RPack>> Where;
        ...
        internal ParameterExpression currentFilterParameter = Expression.Parameter(typeof (RPack));

        internal bool isDistinct, isReduce, all;
        internal Func<IEnumerable<RPack>, IEnumerable<RPack>> solutionModifierOrder;
        ...
        public Func<TripleStore, IEnumerable<IEnumerable<object>>> SelectRun;
        ...
        internal void CreateSelectRun()
        {
            SelectRun = ts =>
            {
                ...
                IEnumerable<RPack> result = Where(Repeat(row, ts));
                
                if (solutionModifierOrder != null) result = solutionModifierOrder(result.Select(i => (RPack)((ICloneable)i).Clone()));
                ...
                return selectResult.Select(seq => seq.Select(o => o));
            };
        }
        ...
        public Func<TripleStore, IEnumerable<IEnumerable<string>>> DescribeRun;
        ...
        internal void CreateDescribeRun()
        {
            DescribeRun = ts =>
            {
                var result = Where != null
                    ? Where(Enumerable.Repeat(new RPack(Variables.Values.Select(variable => variable.pacElement).ToArray(), ts), 1))
                    : Enumerable.Empty<RPack>();
                if (solutionModifierOrder != null) result = solutionModifierOrder(result);

                IEnumerable<string> describeObjects;
                if (all)
                    describeObjects = result.SelectMany(pack => pack.row.Cast<string>());
                else
                    try
                    {
                        describeObjects =
                            variables.Select(s => Variables[s].index).SelectMany(i => result.Select(pack => (string)pack[i]));
                    }
            ...
            //    if (describeObjects.Any(o => o is Literal)) throw new Exception("describe literal");
                ...
                return
                    describeResults.Select(
                        objects => objects.Select(o => o.ToString()));

            };
        }

        public Func<TripleStore, IEnumerable<IEnumerable<object>>> ConstructRun;

        internal void CreateConstructRun()
        {
            ConstructRun = ts =>
            {
                IEnumerable<RPack> result = Where != null
                    ? Where(Enumerable.Repeat(new RPack(Variables.Values.Select(variable => variable.pacElement).ToArray(), ts), 1))
                    : Enumerable.Empty<RPack>();
        ...
        public Func<TripleStore, bool> AsqRun;
        internal Func<RPack, IEnumerable<Tuple<string, string, string>>> constructTemplate;

        internal readonly List<Func<RPack, IEnumerable<Tuple<string, string, string>>>> constructTriples = new List<Func<RPack, IEnumerable<Tuple<string, string, string>>>>();

        internal void CreateAsqRun()
        {
            AsqRun = ts => Where(Enumerable.Repeat(new RPack(Variables.Values.Select(variable => variable.pacElement).ToArray(), ts), 1)).Any();
        }
        ...
        internal Expression Parameter(Variable parameter)
        {
            ...
            if (!parameter.graph.IsData.Value)
               return Expression.Call(currentFilterParameter, typeof (RPack).GetMethod("GetE"), Expression.Constant(parameter.index, typeof (object)));
            return LiteraExpression(Expression.Property(
                Expression.Call(currentFilterParameter, typeof(RPack).GetMethod("Val"), Expression.Constant(parameter.index)),
        ...
        internal Expression Bound(Variable variable)
        {
            if (variable.graph.IsData == null)
                return Expression.Call(currentFilterParameter, typeof (RPack).GetMethod("Hasvalue"),
                    Expression.Constant(variable.index));
            if (!variable.graph.IsData.Value)
                return Expression.NotEqual(Expression.Call(currentFilterParameter, typeof(RPack).GetMethod("GetE"), Expression.Constant((object)variable.index, typeof(object))),
                        Expression.Constant(Int32.MinValue)); 
            else
            switch (variable.graph.vid)
            {
                case LiteralVidEnumeration.integer:
                    return Expression.NotEqual(Expression.Call(currentFilterParameter, typeof(RPack).GetMethod("Vai"),
                ...
                case LiteralVidEnumeration.date:
                    return Expression.NotEqual(Expression.ConvertChecked(
                        Expression.Call(currentFilterParameter, typeof(RPack).GetMethod("Val"), Expression.Constant(variable.index)),
